...
<template>
  <li class="py-2 px-4 first:rounded-t-lg last:rounded-b-lg border-b border-gray-500 last:border-b-0 flex items-center">
    <template v-if="type == 'directory'">
      <FolderIcon />
      <RouterLink class="ml-2 mr-2 transition hover:text-blue-400 hover:underline" :to="calcPath(name)">
        {{ name }}
      </RouterLink>
    </template>
    ...
    <template v-if="type == 'file'">
      <p class="text-gray-400 text-sm ml-auto whitespace-nowrap">{{ prettyBytes(size) }}</p>
