...
<template>
  <nav class="px-5 max-w-screen-md mx-auto py-5">
    <div
      class="w-full bg-widget bg-blur rounded-2xl drop-shadow-sm px-4 py-3 flex items-center justify-between space-x-2"
    >
      <div class="flex items-center space-x-2">
        <img
          class="w-7 h-7 md:w-8 md:h-8 no-select pointer-events-none"
          src="~/assets/images/logo.svg"
          alt="Logo"
        />
        <RouterLink to="/" class="fredoka text-lg md:text-xl inline-block"
          >MyCloud</RouterLink
        >
      </div>
      ...
      <div class="flex items-center space-x-2">
        ...
        <button v-tooltip="auth.user.name">
          <img
            class="w-7 h-7 md:w-8 md:h-8 rounded-lg"
            :src="`${env.public.api}/profileimage/${auth.user.image_url}`"
            :alt="auth.user.name"
          />
        </button>
