...
<template>
  <form class="my-2 text-left" @submit.prevent>
    ...
    <InputUtil :disabled="processing" id="filename" placeholder="File name" v-model="name" :value="name" />
    <label for="directory" class="text-sm text-gray-300 ml-2 mt-3">Directory</label>
    <InputUtil :disabled="processing" id="directory" placeholder="Directory" v-model="directory" :value="directory" />
    <ButtonGreenish class="mt-3" @click="save" :is-loading="processing">Save</ButtonGreenish>
  </form>
