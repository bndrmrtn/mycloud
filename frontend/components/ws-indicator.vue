...
const msg = computed(() => `Realtime connection is ${props.active ? 'active' : 'inactive'}`)
</script>
...
<template>
  ...
  <ModalUtil v-if="show" :show="true" @close="show = false">
    ...
    <p class="text-left mt-2 text-gray-200">
      Our file-sharing service uses WebSocket connections to provide instant notifications.
      This means users receive real-time alerts for uploads, shares, and changes, ensuring they stay updated immediately without needing to refresh or reload.
      🚀
    </p>
  </ModalUtil>
</template>