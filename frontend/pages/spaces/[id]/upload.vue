...
const uploadFileEl = ref<HTMLElement | null>(null);
...
const upload = async () => {
  ...
  if (err == null)
    return process.client ? useToast().success('Successfully uploaded') : null;
  ...
  <SpaceLayout>
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-2">
        <buttons-button-pinkle :to="`/spaces/${id}`" class="!w-min"
          >Files</buttons-button-pinkle
        >
      </div>
    </div>
    ...
    <div class="mt-5 max-w-sm mx-auto">
      <form @submit.prevent>
        ...
        <ButtonBluish
          :disabled="processing"
          @click="(uploadFileEl as HTMLInputElement).click()"
          type="button"
          class="mt-3 md:py-2.5"
        >
