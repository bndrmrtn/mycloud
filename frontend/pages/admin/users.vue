...
<template>
  <SpaceLayout>
    <h1 class="fredoka text-3xl mb-5">Manage {{ isAdmin ? 'admins' : 'users' }}</h1>
    <section class="mt-3">
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
              <img class="w-11 h-11 rounded-xl" :src="env.public.api + '/profileimage/' + user.image_url" alt="">
              <div>
                <h3 class="text-lg fredoka">{{ user.name }}</h3>
                <p class="text-xs text-gray-400">{{ formatDate(user.created_at) }}</p>
              </div>
            </div>
            <ButtonBluish :to="'/admin/users/' + user.id" class="hidden md:block !w-min">Edit</ButtonBluish>
          </div>
          <div class="mt-2">
            <p class="mb-1">
              <span class="fredoka">Email:</span>
              <span class="text-gray-200 bg-main-from px-1.5 py-0.5 rounded-lg m-1 text-sm">{{ user.email }}</span>
            </p>
            <p class="mb-1">
              <span class="fredoka">Role:</span>
              <span class="text-gray-200 bg-main-from px-1.5 py-0.5 rounded-lg m-1 text-sm">{{ user.role }}</span>
              <ShieldIcon v-if="user.role == 'admin'" class="text-yellow-400 -mt-0.5" />
            </p>
            <hr class="border-0 h-1 bg-main-from rounded-lg mb-1">
            <p class="mb-1">
              <span class="fredoka">ID:</span>
              <span class="text-gray-200 bg-main-from px-1.5 py-0.5 rounded-lg m-1 text-sm">{{ user.id }}</span>
            </p>
            <p class="mb-1">
              <span class="fredoka">Google ID:</span>
              <span class="text-gray-200 bg-main-from px-1.5 py-0.5 rounded-lg m-1 text-sm">{{ user.gid }}</span>
            </p>
            <hr class="border-0 h-1 bg-main-from rounded-lg mb-3 mt-2">
            <ButtonBluish :to="'/admin/users/' + user.id" class="mb-2 md:hidden">
              Edit
            </ButtonBluish>
            <ButtonDanger @click="delUser(user.id)" :is-loading="deleting" class="mb-2">
              Delete
            </ButtonDanger>
          </div>
        </li>
      ...
      <div class="max-w-sm mx-auto grid grid-cols-2 gap-2 mb-2 mt-5">
        <ButtonBluish :disabled="cursors.prev_cursor === ''" @click="fetchData(cursors.prev_cursor)">
          Previous
        </ButtonBluish>
        <ButtonBluish :disabled="cursors.next_cursor === ''" @click="fetchData(cursors.next_cursor)">
          Next
        </ButtonBluish>
      </div>
    </section>
